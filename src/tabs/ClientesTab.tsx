...
export function ClientesTab() {
  ...
  const docDigits = String(form.documento || "").replace(/\D/g, "");
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    if (isCnpj) {
      const tipo = String(form.tipo_contribuinte || "")
        .trim()
        .toUpperCase();
      if (!["CONTRIBUINTE", "ISENTO", "NAO_CONTRIBUINTE"].includes(tipo)) {
        setAlertModal({
          open: true,
          title: "Cadastro fiscal",
          message:
            "Para cliente CNPJ, selecione o tipo de contribuinte.",
        });
        return;
      }

      if (tipo === "CONTRIBUINTE" && !String(form.ie || "").trim()) {
        setAlertModal({
          open: true,
          title: "Cadastro fiscal",
          message:
            "Para cliente CNPJ contribuinte, a Inscrição Estadual é obrigatória.",
        });
        return;
      }
    }
    ...
    try {
      const payload: Partial<Cliente> = {
        ...form,
        tipo_contribuinte: isCpf
          ? "NAO_CONTRIBUINTE"
          : String(form.tipo_contribuinte || "NAO_CONTRIBUINTE")
              .trim()
              .toUpperCase(),
        ie: isCpf
          ? ""
          : String(form.tipo_contribuinte || "")
                .trim()
                .toUpperCase() === "ISENTO"
            ? "ISENTO"
            : String(form.tipo_contribuinte || "")
                  .trim()
                  .toUpperCase() === "NAO_CONTRIBUINTE"
              ? ""
              : String(form.ie || ""),
      };

      if (editingId) await adminApi.updateCliente(editingId, payload);
  ...
  const openModal = (c?: Cliente) => {
    ...
    const isCnpjDoc = documentoDigits.length === 14;
    const tipoC =
      String(c?.tipo_contribuinte || "").trim().toUpperCase() ||
      (isCnpjDoc ? "NAO_CONTRIBUINTE" : "NAO_CONTRIBUINTE");
    setForm(
      c
        ? {
            ...c,
            cep: maskCEP(c.cep || ""),
            tipo_contribuinte: tipoC,
            ie: String(c.ie || ""),
          }
        : emptyClienteForm,
    );
                ...
                onChange={(val) => {
                  ...
                  if (digits.length === 14) {
                    setForm({
                      ...
                      documento: val,
                      tipo_contribuinte:
                        form.tipo_contribuinte || "NAO_CONTRIBUINTE",
                    });
                    return;
                  }
                  setForm({ ...form, documento: val });
                }}
              />
            </div>
            <div>
          ...
          {isCnpj && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  className="w-full p-2.5 border rounded-lg"
                  value={String(form.tipo_contribuinte || "NAO_CONTRIBUINTE")}
                  onChange={(e) => {
                ...
                >
                  ...
                  <option value="NAO_CONTRIBUINTE">NAO_CONTRIBUINTE</option>
                </select>
              </div>
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase">
                ...
                <input
                  className={`w-full p-2.5 border rounded-lg ${String(form.tipo_contribuinte || "").toUpperCase() === "NAO_CONTRIBUINTE" ? "bg-slate-100 text-slate-500" : ""}`}
                  value={
                    String(form.tipo_contribuinte || "").toUpperCase() ===
                    "ISENTO"
                      ? "ISENTO"
                      : form.ie || ""
                  }
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <div className="text-slate-700">
                    {selectedDetails.cliente.tipo_contribuinte || "-"}
                  </div>
                  <div className="text-xs text-slate-500 font-mono">
                    {selectedDetails.cliente.ie || "-"}
                  </div>
                </div>
                <div>
