...
export function FinanceiroTab() {
  ...
  const handleCreateConta = async (e: FormEvent) => {
    ...
    try {
      ...
      setNewContaForm({ ...newContaForm, nome: "" });
      setShowNewContaForm(false);
      setContas(await adminApi.getContas({ tipo_uso: "PAGAMENTO" }));
    } catch (e: any) {
      alert(e.message);
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-200">
            <span className="text-sm text-slate-600">
              Contas bancárias utilizadas para pagar despesas.
            </span>
            {!showNewContaForm && (
              <button
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <span
                        className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase ${c.tipo === "Crédito" ? "bg-purple-100 text-purple-700" : "bg-emerald-100 text-emerald-700"}`}
                      >
                        {c.tipo}
                      </span>
                      {c.tipo === "Crédito" && (
